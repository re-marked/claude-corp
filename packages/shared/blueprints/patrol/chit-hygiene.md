...
steps:
  ...
  - id: review-findings
    ...
    description: |
      Run `cc-cli chit list --type kink --status active --tag
      sweeper:chit-hygiene --limit 20` to filter to this patrol's output.

      (Substitute --createdBy if --tag filtering doesn't narrow right; the
      runner sets createdBy='sweeper:chit-hygiene' on every kink it
      emits.)
